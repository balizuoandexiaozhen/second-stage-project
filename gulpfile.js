...
//     return gulp.src('./markdown/md/*.md')
//         .pipe(markdown())//将md文件编译成html
//         .pipe(rename({
//             extname: ".html"
//         }))
//         .pipe(gulp.dest('./markdown/dist')) //gulp.dest() 输出
// })
// gulp.task('watch:md', function () {
//     //可以监听文件的变化之后，指定执行哪些任务
//     gulp.watch( './markdown/md/*.md' , ['compile:md'] )
// })
// gulp.task('default', ['compile:md', 'watch:md'])
...
// //专门给各个页面的html文件添加对应的依赖
// gulp.task('inject', function () {
//         var target = gulp.src('./dist/index/index.html');
//         // It's not necessary to read the files (will speed up things), we're only after their paths:
//         var sources = gulp.src(['./dist/index/js/*.js', './dist/index/css/*.css'], {read: false});
